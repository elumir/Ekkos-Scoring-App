...
const App: React.FC = () => {
  ...
  const mainModes: { mode: SelectionMode; label: string }[] = [
    { mode: 1, label: '1 Winner' },
    { mode: 2, label: '2 Teams' },
  ];
            ...
            >
                ...
                {mainModes.map(({ mode, label }) => (
                    <button
                        ...
                        className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-sky-400 focus:ring-opacity-75 ${
                            selectionMode === mode ? 'bg-sky-500 text-white' : 'bg-transparent text-slate-300 hover:bg-slate-700'
                        }`}
                    >
                        {label}
                    </button>
                ...
                <div className="relative" ref={dropdownRef}>
                    <button
                        ...
                        className={`flex items-center gap-1 px-4 py-2 rounded-full text-sm font-semibold transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-sky-400 focus:ring-opacity-75 ${
                            [3, 4].includes(selectionMode) ? 'bg-sky-500 text-white' : 'bg-transparent text-slate-300 hover:bg-slate-700'
                        }`}
                    >
                        <span>
                            {selectionMode === 3 ? '3 Teams' : selectionMode === 4 ? '4 Teams' : 'More Teams'}
                        </span>
                        <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 transition-transform duration-200 ${isTeamsDropdownOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
